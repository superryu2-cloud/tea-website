...
const SIX_CRITERIA = [
    { key: '泥', en: 'Clay', desc: '泥料是根本。一把好壺的泥料應純正，顆粒豐富。觸摸時應有如豆沙般的細膩感，觀之則光華內斂，光澤能「透」進去，而非浮於表面的「賊光」。' },
    { key: '形', en: 'Form', desc: '器型是靈魂。無論何種造型，都應比例協調、優美大方。整體觀之，要富有精神氣質，即所謂的「精氣神」，站得穩、立得住，給人以美的享受。' },
    { key: '工', en: 'Craftsmanship', desc: '做工是技藝的體現。一把好壺的工藝必然是精湛的，線條流暢，轉折清晰，細節處理到位，鑲接處自然過渡，不拖泥帶水。' },
    { key: '款', en: 'Maker/Seal', desc: '印款是身份的標記。作者的聲譽、師承以及其在壺底、壺蓋處留下的印款，都是評價其價值的重要因素。印款本身也應具有金石篆刻的藝術美感。' },
    { key: '功', en: 'Functionality', desc: '實用性是器物的基本要求。一把壺必須好用，包括：持握舒適、壺蓋嚴密、出水流暢不流口水（無涎水）、斷水果決。' },
    { key: '飾', en: 'Decoration', desc: '裝飾是藝術的升華。如陶刻、泥繪等裝飾手法，其內容、風格必須與壺身造型完美統一，且本身具備相當的藝術水準，起到畫龍點睛的作用。' },
];
...
export default function ZishaAppreciation() {
    return (
        <Section id="zisha-6" title="6．品鑑之道：藏養之樂" icon={Scale}>
            ...
            <SubCard title="選壺六要：鑑賞的準則">
                <p className="mb-4">挑選和評價一把好的紫砂壺，可以從以下六個關鍵維度進行考量，這也是業內公認的「六要」：</p>
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                    {SIX_CRITERIA.map((c, i) => (
                        <div key={c.key} className="rounded-xl border border-stone-100 bg-stone-50/70 p-4">
                            <div className="flex items-baseline gap-2 mb-2">
                                <span className="text-2xl font-extrabold font-serif text-amber-800">{c.key}</span>
                                <span className="text-xs text-stone-400 font-mono">{String(i + 1).padStart(2, '0')} · {c.en}</span>
                            </div>
            ...
            <SubCard title="養壺如養性：人與器的對話">
                ...
                <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50/40 p-4">
                    <div className="text-sm font-extrabold text-amber-900 mb-3">養壺核心要點</div>
